...
We claim the only solution is $f(x)\equiv cx$ for some real constant $c$, which obviously works. We now show it is the only solution.
***

It is easy to check that the only linear function is $f(x)=cx$.
